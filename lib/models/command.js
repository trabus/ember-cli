    ...
    if (option.aliases) {
      option.aliases.forEach(function(alias) {
        ...
        if (typeof alias === 'string') {
          ...
          throw new Error('The "' + alias + '" alias is not an acceptable value. It must be a string or object.')
        }

        if (!self.optionsAliases[key]) {
          self.optionsAliases[key] = value;
        } else {
          throw new Error('The "' + alias + '" alias is already in use by the "' + self.optionsAliases[alias] +
                          '" option for the "' + self.name + '" command. Please use a different alias for the "' +
                          option.name + '" option.');
        }
      });
    }
    console.log('aliases',self.optionsAliases);
  });
}
