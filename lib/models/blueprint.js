...
Blueprint.prototype.install = function(options) {
  ...
  var podPath  = this.podPath = options.podPath = generatePodPath();
...
/*
  Hook to add additional or override fileMapTokenValues.
  The fileMapTokenValues order should match fileMapTokens
  order. Default is expected, pods is necessary if pod structure
  is supported for blueprint.
  @method fileMapTokenValues
  @return {Object}
*/
Blueprint.prototype.fileMapTokenValues = function() {
  return {};
};
...
/*
  @private
  @method _fileMapTokenValues
  @param {Object} options
  @return {Object}
*/
Blueprint.prototype._fileMapTokenValues = function(options) {
  var packageName = options.project.name();
  var moduleName = options.entity && options.entity.name || packageName;
  var dasherizedModuleName = stringUtils.dasherize(moduleName);
  var dasherizedTestName = dasherizedModuleName.replace(/\//,'-');

  var standardTokenValues = {
    default: {
      name: dasherizedModuleName,
      path: this.name + 's',
      test: dasherizedModuleName
    },
    pod: {
      name: this.name,
      path: options.podPath+dasherizedModuleName,
      test: dasherizedTestName
    }
  };
  var customTokenValues = this.fileMapTokenValues(options);
  return merge(standardTokenValues, customTokenValues);
};
...
Blueprint.prototype._locals = function(options) {
  ...
  var fileMapTokens = this._fileMapTokens(options);
  var fileMapTokenValues = this._fileMapTokenValues(options);
  var fileMapLocals = {
    structure: options.structure,
    fileMapTokens: fileMapTokens,
    fileMapTokenValues: fileMapTokenValues
  };
  var fileMap = this.generateFileMap(fileMapLocals);
...
/*
  ...
  @return {Object}
*/
Blueprint.prototype.generateFileMap = function(locals){
  var structure     = locals.structure || 'default';
  var fileMapValues = locals.fileMapTokenValues && locals.fileMapTokenValues[structure] || locals.fileMapTokenValues.default;
  var tokens        = values(locals.fileMapTokens);
  var tokenValues   = values(fileMapValues);
  return zipObject(tokens,tokenValues);
};
/*
